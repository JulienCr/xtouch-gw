...
impl GamepadMapper {
    ...
    /// Handle button event
    ///
    /// Routes both press and release events to the router with the pressed state as value.
    /// This allows actions like `setPtzModifier` to track button state, while actions
    /// like `selectCamera` can ignore release events (value == 0).
    async fn handle_button(control_id: &str, pressed: bool, router: &Arc<Router>) -> Result<()> {
